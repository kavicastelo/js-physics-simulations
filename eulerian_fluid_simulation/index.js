...
function draw()
{
    ...
    if (scene.showStreamlines) {
        ...
        for (i = 1; i < f.numX - 1; i += 5) {
            for (j = 1; j < f.numY - 1; j += 5) {
                ...
                for (n = 0; n < numSegs; n++) {
                    u = f.sampleField(x, y, U_FIELD);
                    v = f.sampleField(x, y, V_FIELD);
                    l = Math.sqrt(u*u + v*v);
                    // x += u/l * segLen;
                    // y += v/l * segLen;
                    x += u * 0.01;
    ...
    if (scene.showPressure) {
        s = "pressure: " + minP.toFixed(0) + " - " + maxP.toFixed(0) + " N/m";
        c.fillStyle ="#000000";
        c.font = "16px Arial";
        c.fillText(s, 10, 35);
    }
